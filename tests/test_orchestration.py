...
from tgemma.translator import HuggingFaceTranslator
...
def _make_hf_translator(mock_tokenizer, max_chunk_tokens=5, batch_size=4):
    """Instantiate HuggingFaceTranslator without loading a model."""
    translator = object.__new__(HuggingFaceTranslator)
    translator.tokenizer = mock_tokenizer
...
def _pipe_batch_output(*texts):
    ...
    return [[{"generated_text": [{"role": "user", "content": ""}, {"role": "assistant", "content": t}]}] for t in texts]


def _pipe_single_output(text):
    """HF pipeline return value for a single-message call: one-item list of dicts."""
    return [{"generated_text": [{"role": "user", "content": ""}, {"role": "assistant", "content": text}]}]


class TestTranslateBatchRetry:
    def test_only_truncated_chunk_is_retried(self, mock_tokenizer):
        """When chunk 2 of 3 is truncated, only chunk 2 triggers a retry; others are untouched."""
        translator = _make_hf_translator(mock_tokenizer, max_chunk_tokens=5)
        ...
        translator.pipe.side_effect = [
            _pipe_batch_output("translated one", truncated_output, "translated three"),  # batch
    ...
    def test_retry_raises_after_max_depth(self, mock_tokenizer):
        ...
        with pytest.raises(TranslationError, match="retry"):
            translator.translate_batch(["Word one two.\n\nWord three four."], "de", "en")
