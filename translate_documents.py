...
def load_tokenizer(model_name: str, fetch: bool = False) -> None:
    ...
    try:
        ...
        if not fetch:
            ...
            print("  Run with --fetch on a node with internet access to download it,")
            print("  or download it manually with:")
            print(f"    uv run hf download {model_name} --include 'tokenizer*' 'special_tokens_map.json'")
            sys.exit(1)
        print("Tokenizer not cached locally, downloading from HuggingFace Hub...")
...
def chunk_text_by_tokens(
    ...
    for para in paragraphs:
        ...
        # Paragraph exceeds token limit => split it into sentences
        if para_tokens > max_tokens:
            ...
            for sent in sentences:
                ...
                if st > max_tokens:
                    # Single sentence exceeds token limit => split by raw token count
                    if sent_chunk:
                    ...
                    # Adding sentence would exceed limit => save and start new chunk
                    chunks.append(' '.join(sent_chunk))
                    sent_chunk = [sent]
                    sent_tokens = st
                else:
                    # Sentence fits => add it to the current sentence chunk
            ...
            # Adding paragraph would exceed limit => save and start new chunk
            chunks.append('\n\n'.join(current_chunk))
            current_chunk = [para]
...
class TranslateGemmaOllama(TranslateGemma):
    ...
    def __init__(
        self, model_name: str = "translategemma:12b", tokenizer=None, max_chunk_tokens: int = MAX_CHUNK_TOKENS
    ):
...
class TranslateGemmaHF(TranslateGemma):
    ...
    def translate(self, text: str, source_lang: str, target_lang: str = "en") -> str:
        ...
        for batch_start in range(0, len(chunks), self.batch_size):
            ...
            outputs = self.pipe(
                text=batch_messages, do_sample=False, batch_size=len(batch),
                pad_token_id=1, max_new_tokens=self.max_chunk_tokens,
            )
...
def translate_file(
    ...
    if detected_lang is None:
        ...
        if detected_lang is None:
            print("  Error: Could not detect language (text may be too short or mixed)")
            print("  Tip: Use --source-lang to specify the language manually")
            return False
    ...
    if not translated or not translated.strip():
        print("  Error: Translation returned empty result")
        return False
    ...
    if len(translated) > 100 and detected_lang != "en":
        ...
        if content[:100] == translated[:100]:
            print("  Warning: Output appears identical to input - translation may have failed")
...
"""
    ...
    if args.chunk_size > MAX_CHUNK_TOKENS:
        print(
            f"Warning: --chunk-size {args.chunk_size} exceeds recommended maximum"
            f" of {MAX_CHUNK_TOKENS} tokens, clamping"
        )
    ...
    if args.backend == "ollama":
        ...
        translator = TranslateGemmaHF(
            model_name, tokenizer=tokenizer, max_chunk_tokens=args.chunk_size,
            batch_size=args.batch_size, use_prompt=args.use_prompt,
        )
